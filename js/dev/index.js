...
function initSliders() {
  if (document.querySelector(".swiper")) {
    let initPortfolioSlider2 = function() {
      const mediaQuery = window.matchMedia("(max-width: 991.98px)");
      if (mediaQuery.matches && !portfolioSwiper) {
        portfolioSwiper = new Swiper(".portfolio__slider", {
          modules: [Navigation],
          observer: true,
          observeParents: true,
          slidesPerView: 1,
      ...
      } else if (!mediaQuery.matches && portfolioSwiper) {
        portfolioSwiper.destroy(true, true);
        portfolioSwiper = null;
      }
    };
    var initPortfolioSlider = initPortfolioSlider2;
    let portfolioSwiper = null;
    initPortfolioSlider2();
    window.addEventListener("resize", initPortfolioSlider2);
  }
}
...
function gsapInit() {
  console.log("gsapInit function called");
  if (!gsapWithCSS || !ScrollTrigger) {
  ...
  console.log("Found sections with [data-fls-gsap]:", animatedSections.length);
  if (animatedSections.length > 0) {
    animatedSections.forEach((section) => {
      const animatedItems = section.querySelectorAll("[data-gsap-item]");
      console.log("Found items in section:", animatedItems.length, section);
      if (animatedItems.length > 0) {
        const animationType = section.getAttribute("data-gsap-type") || "fade-scale";
        const config3 = animationTypes[animationType] || animationTypes["fade-scale"];
        const duration = section.getAttribute("data-gsap-duration") ? parseFloat(section.getAttribute("data-gsap-duration")) : config3.duration;
        const stagger = section.getAttribute("data-gsap-stagger") ? parseFloat(section.getAttribute("data-gsap-stagger")) : typeof config3.stagger === "object" ? config3.stagger : { amount: config3.stagger };
        const ease = section.getAttribute("data-gsap-ease") || config3.ease;
        const start = section.getAttribute("data-gsap-start") || config3.start;
        const fromProps = { ...config3 };
        delete fromProps.duration;
        delete fromProps.stagger;
        delete fromProps.ease;
        delete fromProps.start;
        gsapWithCSS.from(animatedItems, {
          ...fromProps,
          duration,
          stagger,
          ease,
          scrollTrigger: {
            ...
            onEnter: () => console.log("ScrollTrigger activated for section:", section)
          }
        });
      } else {
        console.warn("No items with [data-gsap-item] found in section:", section);
      }
    });
  } else {
    console.warn("No sections with [data-fls-gsap] found");
  }
}
